...
module overmind::NonFungibleToken {
    ...
    */
    public fun mint_nft(
        ...
        assert!(coin::value(payment_coin) >= MIST_PER_SUI, EInsufficientPayment);

        let coin_balance = coin::balance_mut(payment_coin);
        let paid = balance::split(coin_balance, MIST_PER_SUI);

        let mintedNFT = NonFungibleToken {
            id: object::new(ctx),
        ...
        event::emit (NonFungibleTokenMinted { 
            nft_id: object::uid_to_inner(&mintedNFT.id),
            recipient: recipient
        });

        balance::join(&mut minter_cap.sales, paid);
        transfer::transfer({mintedNFT}, recipient);        
    }
    ...
    */
    public fun combine_nfts(
        ...
        string::append_utf8(&mut new_nft_name, b" + ");
        string::append(&mut new_nft_name, nft2.name);

        let new_nft_description = string::utf8(b"Combined NFT of ");
        string::append(&mut new_nft_description, nft1.name);
        string::append_utf8(&mut new_nft_description, b" and ");
        ...
        event::emit({NonFungibleTokenCombined {
            ...
            new_nft_id: object::uid_to_inner(&combined_nft.id)
        }});
    ...
    */
    public fun withdraw_sales(
        ...
        let withdrawn_balance = balance::withdraw_all(&mut minter_cap.sales);
        let withdraw_value = balance::value(&withdrawn_balance);

        event::emit({SalesWithdrawn{ amount: withdraw_value}});
    ...
    */
    public fun burn_nft(nft: NonFungibleToken) {
        event::emit({NonFungibleTokenDeleted{ nft_id: object::uid_to_inner(&nft.id)}});
